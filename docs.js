...
\`\`\`shell
curl 'https://${config.hostname}/locations'
\`\`\`
...
\`\`\`shell
curl 'https://${config.hostname}/stations/nearby'
\`\`\`
...
${profile.trip ? `
	***
	...
	- \`lineName\`: **Required** TODO: What is this?
...
${profile.radar ? `
	***
	...
	${examples.radarCoordinates ? `
		### examples

		\`\`\`shell
		curl 'https://${config.hostname}/radar?${examples.radarCoordinates}'
		\`\`\`
	` : ''}
` : ''}
	`.replace(/\t/g, ''))
